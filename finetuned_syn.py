...
def encode_text_prompts(prompts,model,device="cuda:0"):
...
def dump_data(idx, pred, dmos, dataset):
    import scipy.io as sio
    path = f'./Data/Syn_CVQA/{str(finetune)}'
...
def trainLoop(config, text_encoder, clip_visual, train_set, model, device, criterion, optimizer, logger, writer, epoch, lr_scheduler):
    ...
    for idx, (video, video_lbp, video_dw, label, disType) in enumerate(tqdm(train_set)):
...
def testLoop(config, text_encoder, clip_visual, test_set, model, device, criterion, logger, writer, epoch, checkpath, dataset, disNum=4):
    ...
    beginTime = time.time()
    with torch.no_grad():
        for (img, img_lbp, img_dw, label, disType) in tqdm(test_set):
...
def parse_option():
    parser = argparse.ArgumentParser(
        'NRCVQA for fine-tuned Syn_Cartoon_Video training and evaluation script', add_help=False)
